...
from qiskit_aer.primitives import Estimator as BackendEstimator
...
class QGenerator:
    ...
    def run_batch(self, data_batch: np.ndarray, weight_batch: np.ndarray) -> np.ndarray:
        ...
        # Flatten batch: one circuit per qubit observable
        for b in range(B):
            # Create the parameter bindings for this batch element
        ...
        with Batch(backend=self.backend):
            ...
            for circuit in circuits:
                result = self.estimator.run(circuit, self._z_ops).result()
                results.append(result.values)  
            print(results)

        # Reshape to (B, num_qubits)
        outs = np.array(results, dtype=np.float32).reshape(B, self.num_qubits)
        return outs
...
class GAN(models.Model):
    ...
    def train_step(self, data):
        ...
        res = minimize(
            ...
            x0=w0,
            method="COBYLA",
            options={"maxiter": 50, "rhobeg": 0.1, "tol": 1e-1, "disp": True},
        )
